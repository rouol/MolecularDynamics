...
def glProgramParameteri(program, pname, value):
    ...
    if not _glProgramParameteri:
        ...
        if fptr == 0:
            raise Exception(
                "wglGetProcAddress('glProgramParameteriEXT') returned a zero adress, which will result in a nullpointer error if used.")
...
def glCreateShaderObject(shadertype):
    ...
    if not _glCreateShaderObject:
        ...
        if fptr == 0:
            raise Exception(
                "wglGetProcAddress('glCreateShaderObjectARB') returned a zero adress, which will result in a nullpointer error if used.")
